...
namespace authClient.viewModels
{
    ...
    public class ValidatedValueVM<T> : ValidatedValueVM
    {
        ...
        /// </param>
        /// <returns>
        ///   A <see cref="BoolValue{T}"/> to indicate validation success/failure.
        /// </returns>
        protected virtual BoolValue<T> OnValidateValue(T value)
        {
            var type = typeof(T);
            if (!type.IsValueType)
                return ReferenceEquals(null, value) ? valueIsRequired() : BoolValue<T>.Success(value);

            if (!IsRequired)
                return BoolValue<T>.Success(value);
            ...
            BoolValue<T> valueIsRequired() => BoolValue<T>.Fail($"{ValueName} is required"); ;
        }
        ...
        public ValidatedValueVM(string valueName, ViewModel parent, IServiceProvider services, ILog log) : base(valueName, parent, services, log)
        {
        }
    }
    ...
    public abstract class ValidatedValueVM : ViewModel
    {
        ...
        public ValidatedValueVM(string valueName, ViewModel parent, IServiceProvider services, ILog log) : base(services, log)
        {
            ValueName = valueName;
